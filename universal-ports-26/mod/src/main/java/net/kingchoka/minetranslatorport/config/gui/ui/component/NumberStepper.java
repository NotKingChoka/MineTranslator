...
public final class NumberStepper implements UiControl {
    ...
    @Override public void render(GuiGraphicsExtractor g, Font f, int mx, int my, float pt) {
        ...
        g.centeredText(f,"−",x+11,y+(height-f.lineHeight)/2,UiTheme.MUTED);
        g.centeredText(f,Integer.toString(getter.getAsInt()),x+width/2,y+(height-f.lineHeight)/2,UiTheme.SECONDARY);
        g.centeredText(f,"+",x+width-11,y+(height-f.lineHeight)/2,UiTheme.MUTED);
    }
