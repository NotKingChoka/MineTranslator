...
import net.kingchoka.minetranslator.annotations.ExpectMixin;
import net.kingchoka.minetranslator.compat.clothconfig.MTConfigImplCloth;
import net.kingchoka.minetranslator.core.OpenAIClientProvider;
import net.kingchoka.minetranslator.core.TranslationMode;
import net.kingchoka.minetranslator.core.TranslationService;
...
public interface MTConfig {
    ...
    static void init() {
        if (Platform.isNeoForge()) {
            ...
            MineTranslator.LOGGER.debug("Cloth Config is loaded, using cloth config for Translator++ Config.");
            Dummy.INSTANCE = new MTConfigImplCloth();
            MTConfigImplCloth.init();
        } else {
            MineTranslator.LOGGER.debug("No config API is loaded, using default values for Translator++ Config.");
            Dummy.INSTANCE = new Dummy();
            Dummy.init();
        }
    }

    static MTConfig getInstance() {
        return Dummy.INSTANCE;
    }

    /**
     * The dummy implementation of MTConfig, which uses default values.
     * Only used when no config API is available.
     */
    @ApiStatus.Internal
    class Dummy implements MTConfig {

        /**
         * The instance of the MTConfig, not only works for the dummy one.
         * Modify this field is not allowed.
         * To get this instance, use {@link MTConfig#getInstance()}.
         */
        public static MTConfig INSTANCE;
        ...
        public static void init() {
            ClientTickCallbacks.POST.register(client -> {
                while (MTKeyMappings.CONFIG_KEY.consumeClick()) {
                    ClientUtl.message(client, Component.translatable("misc.MineTranslator.missing.clothconfig"));
                }
            });
        }
    }
