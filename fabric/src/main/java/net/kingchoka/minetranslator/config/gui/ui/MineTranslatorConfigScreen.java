...
import net.kingchoka.minetranslator.config.gui.ui.component.*;
import net.kingchoka.minetranslator.keybind.MTKeyMappings;
import net.kingchoka.minetranslator.translation.ProviderConnectionController;
...
public final class MineTranslatorConfigScreen extends Screen {
    ...
    private final Map<net.minecraft.client.KeyMapping, String> savedKeyMappings = new HashMap<>();
    ...
    public MineTranslatorConfigScreen(Screen parent) {
        ...
        for (var mapping : MTKeyMappings.getEntries()) savedKeyMappings.put(mapping, mapping.saveString());
        buildProviderCards();
    ...
    public boolean mouseDragged(MouseButtonEvent event, double dragX, double dragY) {
        if (scroll.mouseDragged(event.y())) {
    ...
    public boolean keyPressed(KeyEvent event) {
        for (UiControl control : visibleControls()) if (control.keyPressed(event)) return true;
    ...
    public boolean charTyped(CharacterEvent event) {
        for (UiControl control : visibleControls()) if (control.charTyped(event)) return true;
    ...
    private void buildKeys() {
        cards.add(new SettingsCard(Component.translatable("MineTranslator.category.keys"))
            .add(row("MineTranslator.keybind.unified_translate", "MineTranslator.ui.desc.key_unified",
                new KeybindButton(MTKeyMappings.TRANSLATE_KEY, this::markDirty)))
            .add(row("MineTranslator.keybind.toggle_auto_chat", "MineTranslator.ui.desc.key_toggle_chat",
                new KeybindButton(MTKeyMappings.TOGGLE_AUTO_CHAT_KEY, this::markDirty)))
            .add(row("MineTranslator.keybind.toggle_auto_npc", "MineTranslator.ui.desc.key_toggle_npc",
                new KeybindButton(MTKeyMappings.TOGGLE_AUTO_NPC_KEY, this::markDirty)))
            .add(row("MineTranslator.keybind.toggle_auto_items", "MineTranslator.ui.desc.key_toggle_items",
                new KeybindButton(MTKeyMappings.TOGGLE_AUTO_ITEMS_KEY, this::markDirty))));
    }
    ...
    private void buildCache() {
        cards.add(new SettingsCard(Component.translatable("MineTranslator.category.advanced"))
            .add(row("MineTranslator.ui.cache.entries", "MineTranslator.ui.desc.cache_entries",
                new ValuePill(() -> TranslationCache.getInstance().size() + " / " + TranslationCache.getInstance().capacity())))
            .add(row("MineTranslator.ui.cache.hit_rate", "MineTranslator.ui.desc.cache_hit_rate",
                new ValuePill(() -> TranslationCache.getInstance().hitRatePercent() + "%")))
            .add(row("MineTranslator.ui.cache.active", "MineTranslator.ui.desc.cache_active",
                new ValuePill(() -> Integer.toString(TranslationService.getInstance().activeRequests()))))
            .add(row("MineTranslator.ui.cache.queued", "MineTranslator.ui.desc.cache_queued",
                new ValuePill(() -> Integer.toString(TranslationService.getInstance().queuedRequests()))))
    ...
    private void resetSection() {
        ...
        switch (selected) {
            ...
            case KEYS -> {
                for (var mapping : MTKeyMappings.getEntries()) mapping.setKey(mapping.getDefaultKey());
    ...
    private boolean handleConfirmClick(double mouseX, double mouseY, int button) {
        ...
        if (contains(mouseX, mouseY, x + 17 + bw, buttonY, bw, 22)) {
            restoreSavedKeyMappings(); dirty = false; confirmClose = false; minecraft.setScreen(parent); return true;
        }
    ...
    private boolean handleClearCacheClick(double mouseX, double mouseY, int button) {
        ...
        if (contains(mouseX, mouseY, x + dialogW - 188, buttonY, 100, 22)) {
            TranslationCache.getInstance().clear(); confirmClearCache = false; return true;
        }
    ...
    private void copyDiagnostics() {
        ...
        String report = "MineTranslator " + VERSION + "\n"
            ...
            + "Chat interceptors: 1\n"
            + "Tooltip hook: active\n"
            + "Queue: " + TranslationService.getInstance().queuedRequests() + "\n"
            + "Cache: " + TranslationCache.getInstance().size() + "/" + TranslationCache.getInstance().capacity() + "\n"
            + "Last error: none";
        minecraft.keyboardHandler.setClipboard(report);
    }

    private void restoreSavedKeyMappings() {
        savedKeyMappings.forEach((mapping, keyName) -> mapping.setKey(InputConstants.getKey(keyName)));
        net.minecraft.client.KeyMapping.resetMapping();
    }
