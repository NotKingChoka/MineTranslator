package net.kingchoka.minetranslator.event.neoforge;
...
public final class MTEventsImpl {
    ...
    public static void init(IEventBus gameBus, IEventBus modBus) {
        ...
        gameBus.addListener(ScreenEvent.KeyPressed.Pre.class, event -> ScreenCallbacks.KEY_PRESSED_POST.getInvoker().afterKeyPress(event.getScreen(), event.getKeyEvent()));

        ScreenCallbacks.KEY_RELEASED_POST.merge();
        gameBus.addListener(ScreenEvent.KeyReleased.Pre.class, event -> ScreenCallbacks.KEY_RELEASED_POST.getInvoker().afterKeyRelease(event.getScreen(), event.getKeyEvent()));
