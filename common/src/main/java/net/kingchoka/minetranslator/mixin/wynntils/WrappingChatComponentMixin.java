package net.kingchoka.minetranslator.mixin.wynntils;
...
public abstract class WrappingChatComponentMixin extends ChatComponent implements ChatComponentMixinAccessor {
    ...
    public String MineTranslator$getMessageContentAt(double globalMouseX, double globalMouseY) {
        ...
        if (i >= 0 && i < ext.MineTranslator$getMessageIndexTrimmedToAll().length) {
            int idx = ext.MineTranslator$getMessageIndexTrimmedToAll()[i];
