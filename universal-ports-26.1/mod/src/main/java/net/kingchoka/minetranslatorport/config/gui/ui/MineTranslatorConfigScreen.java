...
public final class MineTranslatorConfigScreen extends Screen {
    ...
    private void renderHeader(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ...
        graphics.centeredText(font, "M", x + 23, y + 17, 0xFF171A20);
        graphics.text(font, "MineTranslator", x + 42, y + 11, UiTheme.TEXT, false);
        graphics.text(font, VERSION, x + 42, y + 24, UiTheme.MUTED, false);
        ...
        if (providerX > x + 210) graphics.text(font, provider, providerX, y + 17, UiTheme.SECONDARY, false);
        ...
        graphics.centeredText(font, "x", closeX + 10, y + 14, closeHover ? UiTheme.ERROR : UiTheme.MUTED);
    }

    private void renderSidebar(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
    ...
    private void renderContent(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.text(font, Component.translatable(selected.titleKey), x + padding, y + 12, UiTheme.TEXT, false);
    ...
    private void renderCards(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick, int x, int y, int width) {
        int currentY = y;
    ...
    private void renderProviders(GuiGraphicsExtractor graphics, int mouseX, int mouseY, int x, int y, int width) {
    ...
    private void renderConfirmDialog(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ...
        graphics.text(font, Component.translatable("MineTranslator.dialog.unsaved.title"), x + 14, y + 14, UiTheme.TEXT, false);
    ...
    private void renderClearCacheDialog(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        graphics.fill(0, 0, width, height, 0x99000000);
        int dialogW = Math.min(360, width - 30), dialogH = 100;
        int x = (width - dialogW) / 2, y = (height - dialogH) / 2;
        UiTheme.panel(graphics, x, y, dialogW, dialogH, UiTheme.PANEL);
        graphics.text(font, Component.translatable("MineTranslator.dialog.cache.title"), x + 14, y + 14, UiTheme.TEXT, false);
    ...
    public boolean charTyped(CharacterEvent event) {
        ...
        for (UiControl control : visibleControls()) if (control.charTyped(codePoint, 0)) return true;
    ...
    private boolean handleConfirmClick(double mouseX, double mouseY, int button) {
        ...
        if (contains(mouseX, mouseY, x + 10, buttonY, bw, 22)) {
            save(); confirmClose = false; minecraft.setScreenAndShow(parent); return true;
        }
        if (contains(mouseX, mouseY, x + 17 + bw, buttonY, bw, 22)) {
            dirty = false; confirmClose = false; minecraft.setScreenAndShow(parent); return true;
        }
    ...
    private void drawButton(GuiGraphicsExtractor graphics, Component label, int x, int y, int w, int h,
        ...
        if (font.width(value) <= w - 12) graphics.centeredText(font, value, x + w / 2, y + 7, color);
