...
public final class TranslationKit {
    ...
    public final Map<String, String> translationCache = Collections.synchronizedMap(
            new LinkedHashMap<>(100, 0.75f, true) {
                ...
                protected boolean removeEldestEntry(Map.Entry<String, String> eldest) {
                    return size() > TPPConfig.getInstance().getMaxCacheSize();
                }
            }
    );
    ...
    public void applyChatTranslation(Minecraft client) {
        ...
        if (this.lockedChatMessageIndex >= 0 && this.lockedChatMessageIndex < chat.allMessages.size()) {
            ...
            if (current == this.originalChatMessage || current == this.translatedChatMessage) {
                ...
                ((ChatComponentMixinAccessor) chat).translatorpp$refreshTrimmedMessages();
            }
        }
    }
    ...
    public void releaseChatFreeze(Minecraft client) {
        ...
        if (!this.chatBuffer.isEmpty()) {
            ...
            for (var msg : this.chatBuffer) {
                accessor.translatorpp$addMessageDirect(msg.message, msg.signature, msg.tag);
            }
            this.chatBuffer.clear();
        }

        // Refresh display
        ((ChatComponentMixinAccessor) client.gui.getChat()).translatorpp$refreshTrimmedMessages();
    ...
    public boolean shouldTranslateTooltipLine(TooltipLineType type, TPPConfig config) {
        switch (type) {
            case NAME:
    ...
    public void requestTooltipTranslation(String cleanText, String cacheKey, TPPConfig config, String targetLang) {
    ...
    public void onNewChatMessageAdded(GuiMessage guiMessage, ChatComponent chat) {
        ...
        translationExecutor.submit(() -> {
            try {
                if (finalIsPlayer) {
                    TranslatorPP.LOGGER.info("[Translator++] Player chat detected");
                    TranslatorPP.LOGGER.info("Prefix: {}", finalSplit.prefix.getString());
                    String senderName = getSenderName(finalSplit.prefix.getString());
                    TranslatorPP.LOGGER.info("Username: {}", senderName);
                    TranslatorPP.LOGGER.info("Original content: {}", textToTranslate);
                    TranslatorPP.LOGGER.info("Provider: {}", config.getService().name());
                    TranslatorPP.LOGGER.info("Target language: {}", targetLang);
                }
                ...
                if (translated != null && !translated.isEmpty()) {
                    if (finalIsPlayer) {
                        TranslatorPP.LOGGER.info("Translated content: {}", translated);
                    }
                ...
                TranslatorPP.LOGGER.error("Auto-translation/Preload failed for message: {}. Error: {}", textToTranslate, e.toString());
            }
        });
    }

    private Component assembleTranslatedMessage(SplitComponent split, String origBodyText, String translatedText, boolean isNpc, boolean isPlayer, TPPConfig config) {
    ...
    public boolean onSendChatMessage(String message, ChatScreen screen) {
        ...
        if (message.startsWith("/tppsend ")) {
            ...
            ((ChatScreenMixinAccessor) screen).translatorpp$sendChatDirectly(payload, true);
            return true;
        }
        if (message.startsWith("/tppedit ")) {
            String payload = message.substring("/tppedit ".length());
            ((ChatScreenMixinAccessor) screen).translatorpp$getInput().setValue(payload);
            return true;
        }
                ...
                if (translated != null && !translated.isEmpty()) {
                    Minecraft.getInstance().execute(() -> {
                        if (config.getShowTranslationPreviewBeforeSending()) {
                            ...
                            ((ChatScreenMixinAccessor) screen).translatorpp$sendChatDirectly(translated, true);
                        }
                    });
                } else {
    ...
    private void handleOutboundFailure(String original, ChatScreen screen, Exception e) {
        Minecraft.getInstance().execute(() -> {
            TranslatorPP.LOGGER.error("Outbound translation failed. Error: {}", e.toString());
            TPPConfig config = TPPConfig.getInstance();
            if (config.getSendOriginalIfTranslationFails()) {
                ...
                ((ChatScreenMixinAccessor) screen).translatorpp$sendChatDirectly(original, true);
            } else {
                    ...
                    Minecraft.getInstance().execute(() -> {
                        ...
                        if (config.getAutoSendAfterTranslation()) {
                            ...
                            if (client.screen instanceof ChatScreen chatScreen) {
                                ((ChatScreenMixinAccessor) chatScreen).translatorpp$sendChatDirectly(translated, true);
                            }
                        }
